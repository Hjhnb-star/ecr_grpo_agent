...
\title{ECR-GRPO: Event-Conditioned Credit Refill as a GRPO Plug-in for Asynchronous Long-Horizon LLM Agents}
...
\subsection{Asynchronous feedback is common in agentic RL}
...
\subsection{Reward granularity and reward timing are different issues}
...
\subsection{Event-conditioned credit refill}
...
\begin{enumerate}
\item It formulates asynchronous step credit assignment for long-horizon LLM agents, where feedback arrives as delayed, missing, interrupted, or non-local events.
\item It introduces Event-Conditioned Credit Refill, a mechanism that keeps steps pending and refills their returns when events arrive.
\item It proposes a no-oracle evidence attribution kernel and an event-gated routing rule that separates local feedback from non-local support events.
\item It evaluates the mechanism as a reward-construction module under a fixed GRPO optimizer, reporting both task performance and credit-localization metrics.
\end{enumerate}
...
\subsection{Process and step supervision}
...
\subsection{Long-horizon language agents}
...
\subsection{Delayed reward and semantic credit}
...
\subsection{Step records and asynchronous events}
...
\subsection{Credit refill objective}
...
\subsection{Evidence-conditioned attribution}
...
\subsection{Event-gated routing}
...
\subsection{GRPO-compatible update}
...
\subsection{Empirical variance-reduction intuition}
...
\subsection{Controlled asynchronous diagnostics}
...
\subsection{LLM policy transfer with LoRA}
...
\subsection{External-validity stress protocol}
...
\begin{enumerate}
\item GRPO-Terminal: terminal reward is assigned at the rollout level.
\item GRPO-Uniform: terminal reward is spread uniformly across episode steps.
\item GRPO-Recency: event credit decays with temporal distance.
\item GRPO-Evidence: no-oracle evidence-conditioned attribution.
\item GRPO-Gated: non-terminal events are routed by the delay gate; short-delay events use recency, long-delay events use evidence, and terminal events use the mixture in Section 4.3.
\item Dependency Oracle: exact-link attribution used only as an upper-bound diagnostic.
\end{enumerate}
...
\subsection{Controlled task performance}
...
\subsection{Credit localization}
...
\subsection{Relation to step-level optimization}
